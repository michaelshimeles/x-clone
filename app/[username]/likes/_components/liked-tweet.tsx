...
export default function LikedTweet({ tweet, userProfileId, username, currentUserId }: { tweet: any, userProfileId: string, username: string, currentUserId: string }) {
  ...
  return (
    <Link href={`${tweet?.userInfo?.username}/${tweet?._id}`} key={tweet._id}>
      {isRetweet && (
      ...
      <div className="w-full mx-auto font-sans">
        <div className="">
          <div className="flex items-start space-x-3">
            <img
              src={tweet?.userInfo?.profileImage}
              alt="Profile picture"
              width={48}
              height={48}
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center space-x-1">
                <span className="font-bold text-[15px]">{tweet?.userInfo?.name}</span>
                ...
                <span className="text-gray-500 text-[15px]">
                  @{tweet?.userInfo?.username} · {tweet?._creationTime && formatDistanceToNow(tweet._creationTime)}
                </span>
              </div>
              <p className="mt-1 text-[15px] leading-normal">{tweet?.content}</p>
              {tweet?.userInfo?.images?.length > 0 && (
                <div className="mt-3 grid grid-cols-2 gap-2">
                  {tweet?.userInfo.images.map((image: string, index: number) => (
                    <img
                      key={index}
                      src={image?.split(":")[1]}
              ...
              <div className="mt-3 flex items-center justify-between text-gray-500">
                <div className="flex items-center space-x-4">
                  ...
                  >
                    <Repeat2 className={`w-5 h-5 ${tweet?.retweetCount > 0 && isRetweeted && currentUserId === userProfileId  ? 'fill text-green-500' : 'group-hover:text-green-500'}`} />
                    <span className={`text-xs ${tweet?.retweetCount > 0 && isRetweeted && currentUserId === userProfileId ? 'text-green-500' : 'group-hover:text-green-500'}`}>
